...
const SERVER_PORT_PATH = path.join(CONFIG_DIR, 'server-port.json');
...
async function readServerPort(): Promise<number | null> {
  const state = await readOptionalJson<ServerPortState>(SERVER_PORT_PATH);
...
async function writeServerPort(port: number): Promise<void> {
  await safeWriteJson(SERVER_PORT_PATH, { port });
}
