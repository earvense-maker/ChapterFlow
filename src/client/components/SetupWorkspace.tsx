...
export default function SetupWorkspace({ onCreated, onCancel }: Props) {
  ...
  async function send(text: string) {
    ...
    try {
      ...
      setError(null);
      setPreviewText('');
      const result = await api.sendSetupMessage(session.sessionId, {
        message: trimmed,
  ...
  async function handleCommit() {
    ...
    if (!window.confirm('この相談内容で作品を作成しますか？')) return;
    try {
      setCommitting(true);
      setError(null);
  ...
  async function toggleLock(
    path: string,
    nextLocked: boolean,
    mutateDraftLocked?: (nextDraft: SetupDraft, locked: boolean) => void
  ) {
    if (!session || savingDraft || sending || committing) return;
    ...
    try {
      ...
      if (nextLocked && mutateDraftLocked) {
        const nextDraft = cloneDraft(latest.draft);
        mutateDraftLocked(nextDraft, true);
        const result = await api.updateSetupDraft(latest.sessionId, {
          draft: nextDraft,
      ...
      if (nextLocked) {
        latest = await api.addSetupLock(latest.sessionId, { path, reason: 'user_locked' });
      } else {
        for (const lock of directLocks(latest.locks, path)) {
          latest = await api.removeSetupLock(latest.sessionId, lock.lockId);
        }
        if (mutateDraftLocked) {
          const nextDraft = cloneDraft(latest.draft);
          mutateDraftLocked(nextDraft, false);
          const result = await api.updateSetupDraft(latest.sessionId, {
            draft: nextDraft,
            revision: latest.revision,
          });
    ...
    <div className="setup-workspace">
      <header className="setup-header">
        ...
        <div className="setup-header-actions">
          ...
          <button type="button" onClick={handlePreview} disabled={!session || busy || hasUnsavedDraftEdits}>
            {previewing ? '試し書き中...' : '試し書き'}
          </button>
          <button type="button" className="primary" onClick={handleCommit} disabled={!session || busy || hasUnsavedDraftEdits}>
      ...
      <main className="setup-main">
        <section className="setup-chat" aria-label="相談チャット">
          ...
          <form className="setup-input" onSubmit={handleSubmit}>
            ...
            />
            <button type="submit" className="primary" disabled={sending || committing || hasUnsavedDraftEdits || !message.trim()}>
              {sending ? '相談中...' : '送る'}
            </button>
          </form>
        </section>
          ...
          {draft ? (
            <>
              ...
              <DraftTextList
                ...
                onToggleLock={(item) =>
                  toggleLock(item.id, !pathLocked(item.id, item.locked), (nextDraft, locked) => {
                    const target = nextDraft.confirmed.find((entry) => entry.id === item.id);
              ...
              <DraftCandidateList
                ...
                onToggleLock={(candidate) =>
                  toggleLock(candidate.id, !pathLocked(candidate.id, candidate.locked), (nextDraft, locked) => {
                    const target = nextDraft.candidates.find((entry) => entry.id === candidate.id);
                    if (target) target.locked = locked;
                  })
                }
              ...
              <DraftCharacterList
                ...
                onToggleLock={(character) =>
                  toggleLock(character.id, !pathLocked(character.id, character.locked), (nextDraft, locked) => {
                    const target = nextDraft.characters.find((entry) => entry.id === character.id);
                    if (target) target.locked = locked;
                  })
                }
...
async function createDefaultSetupSession() {
  const providers = await api.getModelProviders().catch(() => [] as ModelProviderInfo[]);
